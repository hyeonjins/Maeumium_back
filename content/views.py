...
class Main2(APIView):
    ...
    def post(self, request):
        ...
        content = Content.objects.create(
            user=user, partner_nickname=partner_user.nickname, start_date=date
        )
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if "thumbnail" in request.FILES:
                ...
                uploaded_thumbnail_url = fs.save(
                    "thumbnails/" + uploaded_thumbnail.name, uploaded_thumbnail
                )
                diary.thumbnail = (
                    uploaded_thumbnail_url  # 저장할 때 전체 경로가 아닌 파일명만 저장하도록 변경
                )
...
class Emotions(APIView):
    def get(self, request):
        ...
        for diary in diaries:
            ...
            # 필터링 함수를 사용하여 필터링된 단어 추출
            # filtered_words = filtering(text)

            # 필터링된 단어를 공백으로 구분하여 문자열로 변환하여 WordCloud 생성
            wordcloud_text = " ".join(text_data)
            ...
            wordcloud_images.append(
                {"diary_id": diary.id, "wordcloud_image_base64": encoded_string}
            )
    ...
    def get(self, request):
        if request.user.is_authenticated:  # 로그인한 사용자인지 확인
            ...
            try:
                ...
                context = {"lover_diaries": lover_diaries}
